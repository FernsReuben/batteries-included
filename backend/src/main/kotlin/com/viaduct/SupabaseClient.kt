...
@Serializable data class SetUserAdminInput(val target_user_id: String, val is_admin: Boolean)

/** Input for searching users RPC call */
@Serializable data class SearchUsersInput(val search_query: String)

/** Input for deleting user RPC call */
@Serializable data class DeleteUserInput(val user_id: String)

/**
 * Request context that can be safely serialized Contains only the user ID, not the authenticated
 * client (which is not serializable)
 */
@Serializable
...
@Serializable data class AuthUserResponse(val id: String, val email: String? = null)
...
open class SupabaseService(
        ...
        private val adminClient: SupabaseClient by lazy {
                createSupabaseClient(supabaseUrl = supabaseUrl, supabaseKey = supabaseKey) {
                        install(Auth) {
                                // Configure Auth module to use longer timeout for token
                                // verification
        ...
        /**
         * Verify a JWT access token with Supabase Auth Returns the user info if valid, throws
        ...
        /**
         * Create an authenticated Supabase client for a specific user This client will use the
         * user's JWT token, enabling RLS policies
         */
        fun createAuthenticatedClient(
                ...
                val client =
                        createSupabaseClient(
                                ...
                                // Provide the user's access token - this sets the Authorization
                                // header for Postgrest
                                // Note: Cannot use install(Auth) with custom accessToken provider
                                // per Supabase SDK
        ...
        /**
         * Helper function to extract authenticated client from request context This should be
         * called by resolvers to get a client for database operations Uses the shared HttpClient
         * for connection pooling
         */
        open fun getAuthenticatedClient(requestContext: Any?): AuthenticatedSupabaseClient {
                val context =
                        requestContext as? GraphQLRequestContext
                                ?: throw IllegalArgumentException(
                                        "Authentication required: invalid or missing request context"
                                )
        ...
        /** Sign in with email and password. Calls Supabase GoTrue API directly. */
        ...
        /** Sign up with email and password. Calls Supabase GoTrue API directly. */
        suspend fun signUp(email: String, password: String): AuthSessionResponse {
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/auth/v1/signup") {
                                header("apikey", supabaseKey)
                                contentType(ContentType.Application.Json)
                                setBody(
                                        json.encodeToString(
        ...
        /** Refresh an access token using a refresh token. Calls Supabase GoTrue API directly. */
        suspend fun refreshToken(refreshToken: String): AuthSessionResponse {
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/auth/v1/token?grant_type=refresh_token") {
                                header("apikey", supabaseKey)
                                contentType(ContentType.Application.Json)
                                setBody(
                                        json.encodeToString(
...
/**
 * Wrapper for an authenticated Supabase client This client uses the user's JWT token, so RLS
 * policies will be enforced automatically
 */
class AuthenticatedSupabaseClient(
        ...
        /**
         * Set a user's admin status Calls the set_user_admin PostgreSQL function Only admins can
         * call this (enforced by the database function)
         */
        ...
        /** Get a user by ID Available to all authenticated users */
        suspend fun getUserById(userId: String): UserEntity? {
                // Call the PostgreSQL RPC function via HTTP
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/rpc/get_user_by_id") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                contentType(ContentType.Application.Json)
        ...
        /** Get all users in the system Only admins can call this */
        suspend fun getAllUsers(): List<UserEntity> {
                // Call the PostgreSQL RPC function via HTTP
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/rpc/get_all_users") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                contentType(ContentType.Application.Json)
                        }
        ...
        /** Search for users by email Available to all authenticated users */
        suspend fun searchUsers(query: String): List<UserEntity> {
                // Call the PostgreSQL RPC function via HTTP
                val input = SearchUsersInput(search_query = query)
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/rpc/search_users") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                contentType(ContentType.Application.Json)
        ...
        /** Delete a user from the system Only admins can call this */
        ...
        /**
         * Get all groups the user is a member of Uses the Supabase Postgrest client which properly
        ...
        /**
         * Get a specific group by ID Uses the Supabase Postgrest client which properly handles RLS
         * policies
         */
        ...
        suspend fun createGroup(
                ...
                val input =
                        com.viaduct.services.CreateGroupInput(
                                name = name,
                                description = description,
                                owner_id = ownerId
                        )
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/groups") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                ...
                                setBody(
                                        json.encodeToString(
                                                com.viaduct.services.CreateGroupInput.serializer(),
                                                input
                                        )
                                )
                        }
                val jsonString = response.bodyAsText()
                val groups =
                        json.decodeFromString<List<com.viaduct.services.CheckboxGroupEntity>>(
                                jsonString
                        )
                return groups.first()
        }
        ...
        suspend fun getGroupMembers(groupId: String): List<com.viaduct.services.GroupMemberEntity> {
                val response: HttpResponse =
                        httpClient.get("$supabaseUrl/rest/v1/group_members") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
        ...
        suspend fun addGroupMember(
                groupId: String,
                userId: String
        ): com.viaduct.services.GroupMemberEntity {
                val input =
                        com.viaduct.services.AddMemberInput(group_id = groupId, user_id = userId)
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/group_members") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                header("Prefer", "return=representation")
                                contentType(ContentType.Application.Json)
                                setBody(
                                        json.encodeToString(
                                                com.viaduct.services.AddMemberInput.serializer(),
                                                input
                                        )
                                )
                        }
                val jsonString = response.bodyAsText()
                val members =
                        json.decodeFromString<List<com.viaduct.services.GroupMemberEntity>>(
                                jsonString
                        )
        ...
        suspend fun getTickets(): List<TicketEntity> {
                return client.from("ticket_resources").select().decodeList<TicketEntity>()
        }
        suspend fun getTicketsByGroup(groupId: String): List<TicketEntity> {
                return client.from("ticket_resources")
                        .select { filter { eq("group_id", groupId) } }
                        .decodeList<TicketEntity>()
        }
        ...
        suspend fun createTicket(
                ...
                val response: HttpResponse =
                        httpClient.post("$supabaseUrl/rest/v1/ticket_resources") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                header("Prefer", "return=representation")
                                contentType(ContentType.Application.Json)
                ...
                val jsonString = response.bodyAsText()
                return json.decodeFromString<List<TicketEntity>>(jsonString).first()
        }
        ...
        suspend fun updateTicket(
                ...
                val input = UpdateTicketInput(id, subject, body, status)

                val response: HttpResponse =
                        httpClient.patch("$supabaseUrl/rest/v1/ticket_resources") {
                                header("Authorization", "Bearer $accessToken")
                                header("apikey", supabaseKey)
                                header("Prefer", "return=representation")
                                contentType(ContentType.Application.Json)
        ...
        suspend fun deleteTicket(id: String): Boolean {
                httpClient.delete("$supabaseUrl/rest/v1/ticket_resources") {
                        header("Authorization", "Bearer $accessToken")
                        header("apikey", supabaseKey)
                        parameter("id", "eq.$id")
                }
